...
def generate_movement(t, phase_shift, step_duration, body_movement, leg_index):
...
while True:
    try:
        ...
        body_movement = (
            p.readUserDebugParameter(vx_slider),
            p.readUserDebugParameter(vy_slider),
            p.readUserDebugParameter(v_rot_slider)
        )
        body_position = (
        ...
        # Open the CSV file in append mode (or write mode if you want to overwrite it)
        with open('leg_pos.csv', 'a', newline='') as csvfile:
            # Define the CSV writer
            csvwriter = csv.writer(csvfile)
            
            # Write the header row only if the file is empty (or if you want to overwrite it with headers)
            # Uncomment the next line if the file is empty and you want to add headers
            # csvwriter.writerow(['t', 'leg_index', 'x', 'y', 'z'])  
            
            # Iterate over all 6 legs
            for leg_index in range(6):
                # Compute trajectory
                pos = generate_movement(t, cpg, step_duration, body_movement, leg_index)  # Pos will generate array [x, y, z]
                leg_base = leg_pos_body[leg_index]
                leg_pos = (leg_base[0] + pos[0], leg_base[1] + pos[1], leg_base[2] + pos[2])
                
                # Log the leg position (time, leg_index, x, y, z)
                csvwriter.writerow([t, leg_index, leg_pos[0], leg_pos[1], leg_pos[2]])
